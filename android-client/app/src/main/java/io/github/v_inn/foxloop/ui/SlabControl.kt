...
private fun DrawScope.drawSlab(image: ImageBitmap?, angleDegrees: Float, panelAspect: Float) {
    ...
    rotate(degrees = angleDegrees, pivot = centre) {
        drawRoundRect(
            color = FoxLoopTokens.Raise,
            topLeft = body.topLeft,
            size = body.size,
            cornerRadius = CornerRadius(chassisCorner),
        )
        drawRoundRect(
            color = FoxLoopTokens.Copper.copy(alpha = 0.3f),
            topLeft = body.topLeft,
            size = body.size,
            cornerRadius = CornerRadius(chassisCorner),
        ...
        drawRect(color = FoxLoopTokens.Recess, topLeft = screen.topLeft, size = screen.size)
...
private fun Readout(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(FoxLoopTokens.SpaceXs)) {
        ...
        BasicText(
            ...
            style = FoxLoopType.eyebrow.copy(
                color = if (staged) FoxLoopTokens.CopperLit else FoxLoopTokens.Graphite,
            ),
        )
        BasicText(
            text = when {
